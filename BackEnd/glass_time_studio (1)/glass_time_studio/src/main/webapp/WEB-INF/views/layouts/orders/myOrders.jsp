...
function loadPage(pageNumber, sortOrder = 'asc', sortBy = 'id') {
    ...
    fetch(apiEndPoint+'/order/allMyOrders/' + memberId + '?page=' + pageNumber + '&size=10')
...
function updateTable(orders) {
    ...
    if (orders.length === 0 || !orders) {
        table.innerHTML = "<tr><td colspan='8'>구매한 내역이 없습니다.</td></tr>";
        return;
    }
    let rows = `<tr>
    ...
    orders.forEach(order => {
        ...
        rows +=
          "<tr>"+
          "<td><a href='/order/"+memberId+"/"+order.orderId+"'>" + order.orderId + "</a></td>" +
          "<td><a href='/order/"+memberId+"/"+order.orderId+"'>" + newDate + "</a></td>" +
          "<td><a href='/order/"+memberId+"/"+order.orderId+"'>" + order.productName + "</a></td>" +
          "<td><a href='/order/"+memberId+"/"+order.orderId+"'>" + order.productQuantity + "</a></td>" +
          "<td><a href='/order/"+memberId+"/"+order.orderId+"'>" + order.orderPayment + "</a></td>" +
          "<td><a href='/order/"+memberId+"/"+order.orderId+"'>" + order.memberName + "</a></td>" +
          "<td><a href='/order/"+memberId+"/"+order.orderId+"'>" + order.address + "</a></td>" +
          "<td><a href='/order/"+memberId+"/"+order.orderId+"'>" + order.orderStatus + "</a></td>" +
          "</tr>";
    });
